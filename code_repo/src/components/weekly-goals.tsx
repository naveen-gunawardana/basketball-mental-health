...
import { Zap, Heart, Brain, Pencil, Target } from "lucide-react";
...
const TABS: { key: Tab; label: string; Icon: React.ComponentType<{ className?: string }> }[] = [
  { key: "effort", label: "Effort", Icon: Zap },
      ...
      {[1, 2, 3, 4, 5].map((n) => (
        <button
          ...
          className={`h-8 w-8 rounded-md text-sm font-semibold transition-colors ${
            value === n
              ? "bg-navy text-white"
              : "bg-offWhite text-navy/60 hover:bg-navy/10"
          }`}
        >
          {n}
        </button>
      ...
      {[1, 2, 3, 4, 5].map((n) => (
        <div
          ...
          className={`h-2 w-6 rounded-full transition-colors ${
            n <= score ? "bg-navy" : "bg-offWhite-300"
          }`}
        />
      ))}
      <span className="text-xs text-muted-foreground ml-1.5 self-center">{score}/5</span>
    </div>
  );
}
        ...
        {editing ? (
          ...
          <>
            ...
            <div className="rounded-lg bg-offWhite p-4">
              ...
              <ScoreDisplay score={goals[`${activeTab}_score` as keyof GoalRow] as number | null} />
            </div>
          </>
        ) : (
          /* ── EMPTY STATE ── */
